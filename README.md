...
CREATE TABLE todos (
    ...
    -- user_id 는 uuid 로써 auth.users 의 id 를 참조하고 사용자 제거시 같이 삭제
    user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
    -- title 컬럼은 비어있으면 안되고, 글자이다.
    title VARCHAR NOT NULL,
    -- completed 는 voolean 으로 기본값이 false 이고 비어있으면 안된다.
...
## 3. 칼럼이 변하거나, 테이블이 추가된다면 type 을 생성하자.

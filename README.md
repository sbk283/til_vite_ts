...
- int4 (Integer): 작은 숫자(-2,147,483,648 ~ 2,147,483,647)
- int8(Big Integer): 아주 큰 숫자 (-9조 ~ 9조)
- float4(Real): 소숫점이 있는 작은 범위(예: 3.14)
- float8(Double Precision): 소숫점이 있는 숫자, 정빌한 값 (예: 3.141592)
- numeric(Decimal): 엄청 정밀한 소숫점 값 (예 : 3.141592)
...
- bool: true, false 값만 가질 수 있음. (예: 주문완료, 주문취소)

## 4. Json 종류 (구조화된 즉 `{키명: 값, 키명:값…}`)

- json: 글자이다. (예: `{이름: 아이유, 나이:18, …}`)
- jsonb: 더 빠르게 검색이 가능한 형태의 바이너리 코드로 변환한 자료형
- 참고사항: Firebase, MongoDB 에서 이 타입을 사용함.

## 5. 독특한 데이터 타입

- uuid: 고유한 ID 를 저장할 때 (`pk` 로 사용)
...
- timestapm: 날짜 + 시간 (예: 2025-09-19 19:09:50+09)
